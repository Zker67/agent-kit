...
system prompts 中提到的外部工具都是宿主侧可选路由规则：宿主环境提供时优先使用，不提供时按宿主自身能力降级。
...
本仓库不内置这些外部工具，也不要求用户安装全部工具才能使用 skills 或模板。若安装了 system prompts，建议按自己宿主的 MCP / CLI 配置补齐 `context7`、`fast-context` 和专业搜索工具，或把提示词里的工具名改成当前宿主实际提供的名字。公开版本已移除图片生成、图像编辑、视频生成和视频编辑的专用路由，避免把个人供应商、模型服务或 API 配置写进公共提示词。
